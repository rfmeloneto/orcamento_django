...
from core.services import project_recurring, delete_projections
...
class TransactionUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_initial(self):
        """Garante que a data atual da transação seja exibida no formulário."""
        initial = super().get_initial()
        initial['date'] = self.object.date
        return initial

    def form_valid(self, form):
        # Captura o estado ANTES de salvar consultando o banco diretamente.
        try:
            old = Transaction.objects.get(pk=form.instance.pk)
            was_recurring = old.is_recurring
        ...
        if not transaction.is_projected and transaction.is_recurring:
            # Transação real marcada como recorrente → projeta meses futuros.
            project_recurring(transaction)
        elif was_recurring and not transaction.is_recurring:
            # Recorrência removida (em qualquer transação, real ou projetada)
            # → apaga as projeções automáticas futuras ligadas a este item.
            # A guarda `is_projected` foi intencionalmente retirada deste ramo:
            # o usuário pode desmarcar a recorrência diretamente em uma entrada
            # projetada exibida na lista, e as projeções seguintes devem sumir.
            delete_projections(transaction)
